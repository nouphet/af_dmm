...
require_once AF_DMM_TRUST_PATH . '/class/AbstractAction.class.php';

define('AF_DMM_FRAME_PERFORM_SUCCESS', 1);
define('AF_DMM_FRAME_PERFORM_FAIL', 2);
define('AF_DMM_FRAME_INIT_SUCCESS', 3);

define('AF_DMM_FRAME_VIEW_NONE','none');
define('AF_DMM_FRAME_VIEW_SUCCESS','success');
define('AF_DMM_FRAME_VIEW_ERROR','error');
define('AF_DMM_FRAME_VIEW_INDEX','index');
define('AF_DMM_FRAME_VIEW_INPUT','input');
define('AF_DMM_FRAME_VIEW_PREVIEW','preview');
define('AF_DMM_FRAME_VIEW_CANCEL','cancel');

/**
 * Af_dmm_Module
**/
class Af_dmm_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Af_dmm_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Af_dmm_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		AF_DMM_FRAME_VIEW_NONE,
		AF_DMM_FRAME_VIEW_SUCCESS,
		AF_DMM_FRAME_VIEW_ERROR,
		AF_DMM_FRAME_VIEW_INDEX,
		AF_DMM_FRAME_VIEW_INPUT,
		AF_DMM_FRAME_VIEW_PREVIEW,
		AF_DMM_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.af_dmm.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'Lists';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('AF_DMM_ADMIN_RENDER_REGISTED'))
		{
			define('AF_DMM_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Af_dmm_AdminRenderSystem' => 'Af_dmm_AdminRenderSystem'
					),
					'Af_dmm_AdminRenderSystem' => array(
						'root' => AF_DMM_TRUST_PATH,
						'path' => '/admin/class/Af_dmmAdminRenderSystem.class.php',
						'class' => 'Af_dmm_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Af_dmm_AdminRenderSystem';
	}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = AF_DMM_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Af_dmm_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Af_dmm_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.af_dmm.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.af_dmm.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.af_dmm.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.af_dmm.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.af_dmm.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.af_dmm.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
		...
		**/
		...
		$root =& XCube_Root::getSingleton();
		$root->mController->executeRedirect(XOOPS_URL, 1, _MD_AF_DMM_ERROR_PERMISSION);
	}
}
