...
class Af_dmm_Utils
{
    ...
    **/
     ...
     * 
     * @return  Af_dmm_AbstractAccessController
    **/
	public static function getAccessControllerObject(/*** string ***/ $dirname, /*** string ***/ $dataname)
	{
		...
		if(! $module){
			require_once AF_DMM_TRUST_PATH . '/class/NoneAccessController.class.php';
			$accessController = new Af_dmm_NoneAccessController($server, $dirname, $dataname);
		...
		switch($role){
		case 'cat':
			require_once AF_DMM_TRUST_PATH . '/class/CatAccessController.class.php';
			$accessController = new Af_dmm_CatAccessController($server, $dirname, $dataname);
			break;
		case 'group':
			require_once AF_DMM_TRUST_PATH . '/class/GroupAccessController.class.php';
			$accessController = new Af_dmm_GroupAccessController($server, $dirname, $dataname);
			break;
		case 'none':
		default:
			require_once AF_DMM_TRUST_PATH . '/class/NoneAccessController.class.php';
    ...
    **/
    public static function getActor(/*** string ***/ $dirname, /*** string ***/ $action)
    {
        ...
        switch($action){
            case Af_dmm_AuthType::VIEW:
                return trim($authType[0]);
                break;
            case Af_dmm_AuthType::POST:
                return trim($authType[1]);
                break;
            case Af_dmm_AuthType::MANAGE:
